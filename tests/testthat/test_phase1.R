test_that("prepare_splits enforces uniqueness and allowed labels", {
  ...
  expect_error(prepare_splits(df2, id_col = "id", split_col = "split"), "must be unique")
})
...
test_that("prepare_events supports single table and list of tables", {
  ...
  out2 <- prepare_events(list(admit=ev1, discharge=ev2), id_col="pid", time_col="t")
  expect_equal(unique(out2$event_type), c("admit","discharge"))
  expect_true(all(out2$source_table %in% c("admit","discharge")))
})

test_that("prepare_observations binds groups and preserves vars", {
