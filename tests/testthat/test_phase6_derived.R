test_that("Phase 6: reconstruct_state_at can add Core-derived variables at anchors", {
  ...
  derived_fns <- list(
    ...
    n_sbp_12 = fluxCore::derive("n_sbp_12", target = fluxCore::declare_variable("sbp"), lookback_t = 12, fn = "count", include_current = FALSE, force = FALSE)
  )
  ...
  row1 <- out[out$entity_id == "p1" & out$t0 == 5, , drop = FALSE]
  ...
  row2 <- out[out$entity_id == "p2" & out$t0 == 0, , drop = FALSE]
